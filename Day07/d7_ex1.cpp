...
void RenderScene(void)
{
	...
	gluLookAt(0.5, 0.5, 0.5, 0, 0, 0, 0, 1.0, 0); //보는 시점이다.
	...
	//glEnable(GL_CULL_FACE);// 함수를 먼저 호출후
	//glCullFace(GL_BACK); //이렇게 안보이는 부분을 파낼수있다.
...
void ChangeSize(int w, int h)
{
	...
	//xMin, xMax, yMin,yMax //오른손 좌표계라 zMin zMax 작아질수록 멀어 지는거다.
	//그래서 -zMax , -zMin 이런식으로 좌표가 표시 되어있는거같애
	else
