...
int rotateAxis = 1;//0 , 1, 2 x, y, z 순으로 된다.
int rotateVector[3][3] = {
	{1,0,0},{0,1,0}, {0,0,1}
};

int charAxis[3] = { 'X','Y','Z' };
bool clicked = false;
...
void RenderScene(void)
{
	...
	if (clicked)
	{
		...
		glPrint("Rotate Axis [ %c ]", charAxis[rotateAxis]);
	}
	glRotatef(theta, rotateVector[rotateAxis][0], rotateVector[rotateAxis][1], rotateVector[rotateAxis][2]);
	drawCube();
