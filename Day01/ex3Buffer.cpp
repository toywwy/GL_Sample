//버퍼를 번갈아 가면서 사용함으로서 그 깜빡 깜박 거리는 문제를 해결 할 수 있다.기본적으로 더블 버퍼링 다 지원해야한ㄷㅏ.
//ㅂㅓ퍼가 화면 뿌리는 도트를 저장해야 하기 때문에 비디오 메모리가 커야한다.ㅈㅗ그만 화면인 경우에는 문제 안되지만 uhd같은거 표현하려면 커야한다.
//그냥 이제 glFlush로 밀어버리는게 아니라 버퍼 A B를 스왑하는 방식으로 한다고 RenderScene에 추가해주고
//메인에서 displayMode에서 변경을해준다.
